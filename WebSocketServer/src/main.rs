...
//         // 🆕 ADD THIS: Send current active users immediately after init
//         let users = {
//             let mut seen = HashSet::new();
...
//         if !users.is_empty() {
//             let active_users_msg = ServerSocketMessage::ActiveUsers { users };
//             let _ = user_ws_tx.send(Message::text(serde_json::to_string(&active_users_msg).unwrap())).await;
//         }
//     }
